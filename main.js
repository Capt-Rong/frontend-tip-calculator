...
const billInput = document.querySelector('.splitter__input_bill')
const peopleInput = document.getElementById('people') || document.querySelector('.splitter__input_person');
...
const customField = document.getElementById('custom') || document.querySelector('.splitter__input_tip');
...
function colorReset() {
    tipButtons.forEach(btn => btn.classList.remove('splitter__tip-item_pushed'));
}
...
customField.addEventListener('input', (e) => {
    if (e.target.value > percentLimit) e.target.value = percentLimit = Number(customField.value);
...
tipButtons.forEach(button => {
    button.addEventListener('click', (event) => {
        ...
        if (input === 'custom') {
            ...
            tipValue = Number(customField.value) || 0;
        } else {
...
resetButton.addEventListener('click', () => {
    ...
    peopleInput.classList.remove('splitter__input_outline');
    resetButton.setAttribute('disabled', '');
    resetButton.classList.remove('splitter__main-button_enabled', 'splitter__main-button_hovered');
    billInput.focus();
});
